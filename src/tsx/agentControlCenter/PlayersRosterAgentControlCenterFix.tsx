...
// Обновленная функция для форматирования с поддержкой текста
const formatValueWithSpaces = (value: string | number | null): string => {
...
// Обновленная функция для преобразования обратно в базу данных
...
// Обновленная функция для применения маски ввода с поддержкой текста
const applyInputMask = (value: string, previousValue: string): string => {
...
const PlayersRosterAgentControlCenter = () => {
  ...
  // Обновленная функция handleMassShift с учетом текста
  const handleMassShift = async () => {
    ...
    try {
      const updatedPlayers = players.map((player) => {
        ...
        // Для каждого year поля парсим значение
      ...
      const { error } = await supabase.from("Players").upsert(updatedPlayers);
  ...
  const handleTeamCellClick = (e: React.MouseEvent, playerId: number) => {
  ...
  const handleTeamSelect = (team: string) => {
    ...
    setTeamContextMenu({ show: false, x: 0, y: 0, playerId: null });
  };
  ...
  const handleGLeagueCellClick = (e: React.MouseEvent, playerId: number) => {
    if (editingId === playerId || isAddingNew) {
      e.preventDefault();
  ...
  const handleGLeagueSelect = (g_league: string) => {
    if (gLeagueContextMenu.playerId) {
      if (isAddingNew) {
    ...
    setGLeagueContextMenu({ show: false, x: 0, y: 0, playerId: null });
  };

  const handleCloseGLeagueMenu = () => {
    setGLeagueContextMenu({ show: false, x: 0, y: 0, playerId: null });
  };
  ...
  const filteredGLeagueTeams = G_LEAGUE_TEAMS.filter((team) =>
  ...
  // Обновленная функция handleEdit с поддержкой текста
  const handleEdit = (player: Player) => {
    setEditingId(player.id);

    // Создаем копию данных игрока с отформатированными значениями
  ...
  // Обновленная функция handleSave с поддержкой текста
  const handleSave = async () => {
    if (!editingId) return;

    // Создаем копию editData с распарсенными значениями
  ...
  // Обновленный обработчик изменения для editData с поддержкой текста
  const handleEditDataChange = (field: keyof Player, value: string) => {
    let formattedValue = value;

    // Если это year колонка, применяем новую маску
  ...
  // Обновленный обработчик изменения для newPlayer с поддержкой текста
  ...
  // Обновленная функция handleAddNew с поддержкой текста
  const handleAddNew = async () => {
    // Создаем копию newPlayer с распарсенными значениями
