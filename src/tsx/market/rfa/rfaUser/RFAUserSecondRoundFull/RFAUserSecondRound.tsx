...
const RFAUserSecondRound: React.FC = () => {
  ...
  const handleSubmit = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      // ИЗМЕНЕНИЕ: Заменена таблица сохранения
      const { error } = await supabase
        .from("Market_RFA_second_round")
        .insert([dataToInsert]);

      if (error) throw error;

      setSubmitSuccess(true);
      console.log("Данные успешно сохранены в таблицу Market_RFA_second_round");
  ...
  return (
    <div className="rfaUserSecondRoundSpaceMadeOffers-container">
      <div className="rfaUserSecondRoundSpaceMadeOffers-content">
        {/* Заголовок секции */}
        <div className="rfaUserSecondRoundSpaceMadeOffers-section-header">
          <div className="rfaUserSecondRoundSpaceMadeOffers-title-wrapper">
            <h2 className="rfaUserSecondRoundSpaceMadeOffers-main-title">
              СОЗДАНИЕ ПРЕДЛОЖЕНИЯ RFA
            </h2>
            <p className="rfaUserSecondRoundSpaceMadeOffers-subtitle">
              Заполните все поля для подачи предложения
            </p>
            <div className="rfaUserSecondRoundSpaceMadeOffers-divider"></div>
          </div>
        </div>

        {/* Сообщения об ошибке/успехе */}
        {submitError && (
          <div className="rfaUserSecondRoundSpaceMadeOffers-error-message">
            {submitError}
          </div>
        )}

        {submitSuccess && (
          <div className="rfaUserSecondRoundSpaceMadeOffers-success-message">
            Предложение успешно отправлено!
          </div>
        )}

        {/* Выбор команды */}
        <div className="rfaUserSecondRoundSpaceMadeOffers-section">
          <h3 className="rfaUserSecondRoundSpaceMadeOffers-section-title">
            Выберите команду NBA
          </h3>
          <div
            className="rfaUserSecondRoundSpaceMadeOffers-dropdown-wrapper"
            ref={teamDropdownRef}
          >
            <button
              className={`rfaUserSecondRoundSpaceMadeOffers-toggle ${selectedTeam ? "has-selection" : ""} ${isTeamOpen ? "active" : ""}`}
              onClick={toggleTeamDropdown}
              disabled={loadingTeams}
              type="button"
            >
              <span className="rfaUserSecondRoundSpaceMadeOffers-button-text">
                {getTeamButtonText()}
              </span>
              {selectedTeam && (
                <button
                  className="rfaUserSecondRoundSpaceMadeOffers-clear"
              ...
              <span className="rfaUserSecondRoundSpaceMadeOffers-arrow">
                {isTeamOpen ? "▲" : "▼"}
              </span>
            </button>

            {teamError && (
              <div className="rfaUserSecondRoundSpaceMadeOffers-error">
                {teamError}
              </div>
            )}

            {isTeamOpen && (
              <div className="rfaUserSecondRoundSpaceMadeOffers-menu">
                {loadingTeams ? (
                  <div className="rfaUserSecondRoundSpaceMadeOffers-loading">
                    <div className="rfaUserSecondRoundSpaceMadeOffers-spinner"></div>
                    <span>Загрузка команд...</span>
                  </div>
                ) : teams.length === 0 ? (
                  <div className="rfaUserSecondRoundSpaceMadeOffers-empty">
                    Команды не найдены
                  </div>
                ) : (
                  <div className="rfaUserSecondRoundSpaceMadeOffers-list-wrapper">
                    <ul className="rfaUserSecondRoundSpaceMadeOffers-list">
                      {teams.map((team) => (
                        <li
                          key={team.id}
                          className={`rfaUserSecondRoundSpaceMadeOffers-item ${selectedTeam?.id === team.id ? "selected" : ""}`}
                          onClick={() => handleTeamSelect(team)}
                        >
                          <span className="rfaUserSecondRoundSpaceMadeOffers-item-name">
                            {team.team_name}
                          </span>
                          {selectedTeam?.id === team.id && (
                            <span className="rfaUserSecondRoundSpaceMadeOffers-checkmark">
                              ✓
                            </span>
                          )}
                        </li>
        ...
        <div className="rfaUserSecondRoundSpaceMadeOffers-section">
          ...
          >
            <button
              className={`rfaUserSecondRoundSpaceMadeOffers-toggle ${selectedPlayer ? "has-selection" : ""} ${isPlayerOpen ? "active" : ""}`}
                  ...
                  <div className="rfaUserSecondRoundSpaceMadeOffers-list-wrapper">
                    <ul className="rfaUserSecondRoundSpaceMadeOffers-list">
                      {players.map((player) => (
                        <li
                          key={player.id}
                          className={`rfaUserSecondRoundSpaceMadeOffers-item ${selectedPlayer?.id === player.id ? "selected" : ""}`}
        ...
        <div className="rfaUserSecondRoundSpaceMadeOffers-section">
          ...
          <div className="rfaUserSecondRoundSpaceMadeOffers-input-section">
            {/* Блок с минимальными суммами по стажу - ПЕРЕМЕЩЕН ВВЕРХ */}
            <div className="rfaUserSecondRoundSpaceMadeOffers-minimum-salaries">
              <div className="rfaUserSecondRoundSpaceMadeOffers-minimum-title">
                Минимальная сумма контракта по стажу (тыс.):
              </div>
              <div className="rfaUserSecondRoundSpaceMadeOffers-minimum-grid">
                {MINIMUM_SALARY_NOTES.map((item, index) => (
                  <div
                    key={index}
                    className="rfaUserSecondRoundSpaceMadeOffers-minimum-item"
                  >
                    <span className="rfaUserSecondRoundSpaceMadeOffers-minimum-experience">
                      {item.experience} год:
                    </span>
                    <span className="rfaUserSecondRoundSpaceMadeOffers-minimum-salary">
                      {item.salary}
                    </span>
                  </div>
                ))}
              </div>
              <div className="rfaUserSecondRoundSpaceMadeOffers-minimum-note">
                * Минимальная сумма зависит от стажа игрока в NBA
              </div>
            </div>

            {/* Поле ввода суммы контракта */}
            <div className="rfaUserSecondRoundSpaceMadeOffers-input-wrapper">
              <input
                ref={contractInputRef}
                type="text"
                className="rfaUserSecondRoundSpaceMadeOffers-input"
              ...
              {contractAmount && (
                <button
                  className="rfaUserSecondRoundSpaceMadeOffers-input-clear"
            ...
            <div className="rfaUserSecondRoundSpaceMadeOffers-input-hint">
            ...
            {contractAmount &&
              selectedContractType &&
              selectedContractLength && (
                <div className="rfaUserSecondRoundSpaceMadeOffers-yearly-amounts">
                  <div className="rfaUserSecondRoundSpaceMadeOffers-yearly-title">
                    Сумма контракта по годам ({selectedContractType.label}):
                  </div>
                  <div className="rfaUserSecondRoundSpaceMadeOffers-yearly-list">
                    {yearlyContractAmounts.map((amount, index) => {
                      ...
                      return (
                        <div
                          key={index}
                          className="rfaUserSecondRoundSpaceMadeOffers-yearly-item"
                        >
                          <span className="rfaUserSecondRoundSpaceMadeOffers-year-label">
                            Год {index + 1}
                            {optionLabel && (
                              <span className="rfaUserSecondRoundSpaceMadeOffers-year-option">
                                {optionLabel}
                              </span>
                            )}
                          </span>
                          <span className="rfaUserSecondRoundSpaceMadeOffers-year-amount">
                            {formatAmount(amount)} тыс.
                          </span>
                          {index > 0 && (
                            <span className="rfaUserSecondRoundSpaceMadeOffers-year-change">
                              {selectedContractType.id === "flat" ? (
                                "без изменений"
                              ) : selectedContractType.id === "front" ? (
                                <span className="rfaUserSecondRoundSpaceMadeOffers-decrease">
                                  ▼ 5%
                                </span>
                              ) : selectedContractType.id === "back" ? (
                                <span className="rfaUserSecondRoundSpaceMadeOffers-increase">
                                  ▲ 5%
                                </span>
                              ) : (
                                <span className="rfaUserSecondRoundSpaceMadeOffers-increase">
                    ...
                    {yearlyContractAmounts.length > 0 && (
                      <div className="rfaUserSecondRoundSpaceMadeOffers-total-amount">
                        <span className="rfaUserSecondRoundSpaceMadeOffers-total-label">
                          Общая сумма:
                        </span>
                        <span className="rfaUserSecondRoundSpaceMadeOffers-total-value">
                          {formatAmount(
        ...
        <div className="rfaUserSecondRoundSpaceMadeOffers-section">
          ...
          >
            <button
              className={`rfaUserSecondRoundSpaceMadeOffers-toggle ${selectedContractType ? "has-selection" : ""} ${isContractTypeOpen ? "active" : ""}`}
                ...
                <div className="rfaUserSecondRoundSpaceMadeOffers-list-wrapper">
                  <ul className="rfaUserSecondRoundSpaceMadeOffers-list">
                    {CONTRACT_OPTIONS.map((option) => (
                      <li
                        key={option.id}
                        className={`rfaUserSecondRoundSpaceMadeOffers-item ${selectedContractType?.id === option.id ? "selected" : ""}`}
                        onClick={() => handleContractTypeSelect(option)}
                      >
                        <div className="rfaUserSecondRoundSpaceMadeOffers-contract-option">
                          <div className="rfaUserSecondRoundSpaceMadeOffers-contract-label">
                            {option.label}
                          </div>
                          <div className="rfaUserSecondRoundSpaceMadeOffers-contract-description">
                            {option.description}
                          </div>
                        </div>
        ...
        <div className="rfaUserSecondRoundSpaceMadeOffers-section">
          ...
          >
            <button
              className={`rfaUserSecondRoundSpaceMadeOffers-toggle ${selectedContractLength ? "has-selection" : ""} ${isContractLengthOpen ? "active" : ""}`}
                ...
                <div className="rfaUserSecondRoundSpaceMadeOffers-list-wrapper">
                  <ul className="rfaUserSecondRoundSpaceMadeOffers-list">
                    {CONTRACT_LENGTH_OPTIONS.map((option) => (
                      <li
                        key={option.id}
                        className={`rfaUserSecondRoundSpaceMadeOffers-item ${selectedContractLength?.id === option.id ? "selected" : ""}`}
        ...
        <div className="rfaUserSecondRoundSpaceMadeOffers-section">
          ...
          >
            <button
              className={`rfaUserSecondRoundSpaceMadeOffers-toggle ${selectedContractClause ? "has-selection" : ""} ${isContractClauseOpen ? "active" : ""}`}
                ...
                <div className="rfaUserSecondRoundSpaceMadeOffers-list-wrapper">
                  <ul className="rfaUserSecondRoundSpaceMadeOffers-list">
                    {CONTRACT_CLAUSE_OPTIONS.map((option) => (
                      <li
                        key={option.id}
                        className={`rfaUserSecondRoundSpaceMadeOffers-item ${selectedContractClause?.id === option.id ? "selected" : ""}`}
        ...
        <div className="rfaUserSecondRoundSpaceMadeOffers-section">
          ...
          >
            <button
              className={`rfaUserSecondRoundSpaceMadeOffers-toggle ${selectedSigningCondition ? "has-selection" : ""} ${isSigningConditionOpen ? "active" : ""}`}
                ...
                <div className="rfaUserSecondRoundSpaceMadeOffers-list-wrapper">
                  <ul className="rfaUserSecondRoundSpaceMadeOffers-list">
                    {SIGNING_CONDITION_OPTIONS.map((option) => (
                      <li
                        key={option.id}
                        className={`rfaUserSecondRoundSpaceMadeOffers-item ${selectedSigningCondition?.id === option.id ? "selected" : ""}`}
        ...
        <div className="rfaUserSecondRoundSpaceMadeOffers-section rfaUserSecondRoundSpaceMadeOffers-submit-section">
          <button
            className="rfaUserSecondRoundSpaceMadeOffers-submit-button"
...
export default RFAUserSecondRound;
