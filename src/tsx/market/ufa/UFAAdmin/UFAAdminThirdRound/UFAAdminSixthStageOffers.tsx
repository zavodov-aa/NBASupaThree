...
import "./ufaAdminSixthStageOffers.css";
import { supabase } from "../../../../../Supabase";

interface UFASixthStageOffer {
  team: string;
  player: string;
...
const UFAAdminSixthStageOffers = () => {
  const [offers, setOffers] = useState<UFASixthStageOffer[]>([]);
  ...
  if (loading)
    return (
      <div className="ufaAdminSixthStageOffers-loading">Загрузка данных...</div>
    );
  if (error)
    return (
      <div className="ufaAdminSixthStageOffers-error">Ошибка: {error}</div>
    );

  return (
    <div className="ufaAdminSixthStageOffers">
      <h2 className="ufaAdminSixthStageOffers-title">
        Предложения UFA (шестой этап второго раунда)
      </h2>

      <div className="ufaAdminSixthStageOffers-filtersPanel">
        <div className="ufaAdminSixthStageOffers-filterGroup">
          <label
            htmlFor="team-filter"
            className="ufaAdminSixthStageOffers-filterLabel"
          >
            Фильтр по команде:
          </label>
          <input
            ...
            className="ufaAdminSixthStageOffers-filterInput"
        ...
        <button
          ...
          className="ufaAdminSixthStageOffers-clearFiltersBtn"
          aria-label="Сбросить все фильтры и сортировку"
        >
          Сбросить фильтры
        </button>

        <div className="ufaAdminSixthStageOffers-resultsCount">
          Найдено: {filteredAndSortedOffers.length} из {offers.length}
        </div>
      </div>

      <div className="ufaAdminSixthStageOffers-tableWrapper">
        <table className="ufaAdminSixthStageOffers-table">
          <thead>
            <tr>
              <th
                onClick={() => handleSort("team")}
                className="ufaAdminSixthStageOffers-sortableHeader"
                aria-label="Сортировать по команде"
              >
                <span className="ufaAdminSixthStageOffers-headerContent">
                  Команда{" "}
              ...
              <th className="ufaAdminSixthStageOffers-header">
                <span className="ufaAdminSixthStageOffers-headerContent">
                  Тип контракта
                </span>
              </th>
            ...
            {filteredAndSortedOffers.length > 0 ? (
              filteredAndSortedOffers.map((offer, index) => (
                <tr
                  ...
                  className="ufaAdminSixthStageOffers-tableRow"
                >
                  <td
                    className="ufaAdminSixthStageOffers-tableCell"
                    data-label="Команда"
                  >
                    {offer.team}
                  </td>
              ...
              <tr className="ufaAdminSixthStageOffers-tableRow">
                <td
                  colSpan={13}
                  className="ufaAdminSixthStageOffers-noDataCell"
                >
...
export default UFAAdminSixthStageOffers;
