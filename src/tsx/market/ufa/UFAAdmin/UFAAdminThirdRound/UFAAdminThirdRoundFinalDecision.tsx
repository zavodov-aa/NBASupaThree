...
// Интерфейс строки данных из таблицы Market_UFA_third_round_result
...
const UFAAdminThirdRoundFinalDecision = () => {
  ...
  // Загрузка данных из таблицы Market_UFA_third_round_result
  const fetchData = async () => {
    ...
    try {
      const { data: fetchedData, error } = await supabase
        ...
        .select(
          "id, player, ufa_third_round_first_stage_result, ufa_third_round_second_stage_result, final_third_round_decision",
        );

      if (error) throw error;
      setData((fetchedData as UfaThirdRoundResultRow[]) || []);
    } catch (err) {
  ...
  const updateFinalDecision = async (id: number, team: string) => {
    try {
      const { error } = await supabase
        .from("Market_UFA_third_round_result")
        .update({ final_third_round_decision: team })
        .eq("id", id);
      if (error) throw error;
    } catch (err) {
  ...
  const handleTeamSelect = async (rowId: number, team: string) => {
    ...
    setData((prevData) =>
      prevData.map((row) =>
        row.id === rowId ? { ...row, final_third_round_decision: team } : row,
      ),
    );
    // Сохраняем в БД
  ...
  return (
    <div style={{ padding: "20px" }}>
      <h1>Финальное решение по игрокам (третий раунд UFA)</h1>
            ...
            >
              <thead>
                <tr>
                  ...
                  <th style={{ border: "1px solid #ccc", padding: "8px" }}>
                    1-й этап 3-го раунда UFA
                  </th>
                  <th style={{ border: "1px solid #ccc", padding: "8px" }}>
                    2-й этап 3-го раунда UFA
                  </th>
                  <th style={{ border: "1px solid #ccc", padding: "8px" }}>
                    Финальное решение
                  </th>
                ...
                {data.length > 0 ? (
                  data.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td style={{ border: "1px solid #ccc", padding: "8px" }}>
                        {row.ufa_third_round_first_stage_result}
                      </td>
                      <td style={{ border: "1px solid #ccc", padding: "8px" }}>
                        {row.ufa_third_round_second_stage_result}
                      </td>
                      <td
                        style={{
                          border: "1px solid #ccc",
                      ...
                      >
                        {editingRowId === row.id ? (
                          <select
                            value={row.final_third_round_decision || ""}
                            onChange={(e) =>
                          ...
                          <span style={{ display: "block", width: "100%" }}>
                            {row.final_third_round_decision || "—"}
                          </span>
                        )}
                      </td>
                    </tr>
...
export default UFAAdminThirdRoundFinalDecision;
