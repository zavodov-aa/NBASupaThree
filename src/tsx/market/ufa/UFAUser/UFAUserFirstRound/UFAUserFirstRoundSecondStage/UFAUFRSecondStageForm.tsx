...
import "./ufaufrSecondStageForm.css"; // CSS для второго этапа
...
const UFAUFRSecondStageForm: React.FC = () => {
  ...
  const handleSubmit = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      // ИЗМЕНЕНО: таблица для второго этапа
      ...
      console.log(
        "Данные успешно сохранены в таблицу Market_UFA_first_round_second_stage",
      );
  ...
  return (
    <div className="ufaUFRSecondStageForm-container">
      <div className="ufaUFRSecondStageForm-content">
        {/* Заголовок секции */}
        <div className="ufaUFRSecondStageForm-section-header">
          <div className="ufaUFRSecondStageForm-title-wrapper">
            <h2 className="ufaUFRSecondStageForm-main-title">
              СОЗДАНИЕ ПРЕДЛОЖЕНИЯ UFA (ПЕРВЫЙ РАУНД, ВТОРОЙ ЭТАП)
            </h2>
            <p className="ufaUFRSecondStageForm-subtitle">
              Заполните все поля для подачи предложения
            </p>
            <div className="ufaUFRSecondStageForm-divider"></div>
          </div>
        </div>

        {submitError && (
          <div className="ufaUFRSecondStageForm-error-message">
            {submitError}
          </div>
        )}

        {submitSuccess && (
          <div className="ufaUFRSecondStageForm-success-message">
            Предложение успешно отправлено!
          </div>
        )}

        {/* Выбор команды */}
        <div className="ufaUFRSecondStageForm-section">
          <h3 className="ufaUFRSecondStageForm-section-title">
            Выберите команду NBA
          </h3>
          <div
            className="ufaUFRSecondStageForm-dropdown-wrapper"
            ref={teamDropdownRef}
          >
            <button
              className={`ufaUFRSecondStageForm-toggle ${selectedTeam ? "has-selection" : ""} ${isTeamOpen ? "active" : ""}`}
              onClick={toggleTeamDropdown}
              disabled={loadingTeams}
              type="button"
            >
              <span className="ufaUFRSecondStageForm-button-text">
                {getTeamButtonText()}
              </span>
              {selectedTeam && (
                <button
                  className="ufaUFRSecondStageForm-clear"
              ...
              <span className="ufaUFRSecondStageForm-arrow">
                {isTeamOpen ? "▲" : "▼"}
              </span>
            </button>

            {teamError && (
              <div className="ufaUFRSecondStageForm-error">{teamError}</div>
            )}

            {isTeamOpen && (
              <div className="ufaUFRSecondStageForm-menu">
                {loadingTeams ? (
                  <div className="ufaUFRSecondStageForm-loading">
                    <div className="ufaUFRSecondStageForm-spinner"></div>
                    <span>Загрузка команд...</span>
                  </div>
                ) : teams.length === 0 ? (
                  <div className="ufaUFRSecondStageForm-empty">
                    Команды не найдены
                  </div>
                ) : (
                  <div className="ufaUFRSecondStageForm-list-wrapper">
                    <ul className="ufaUFRSecondStageForm-list">
                      {teams.map((team) => (
                        <li
                          key={team.id}
                          className={`ufaUFRSecondStageForm-item ${selectedTeam?.id === team.id ? "selected" : ""}`}
                          onClick={() => handleTeamSelect(team)}
                        >
                          <span className="ufaUFRSecondStageForm-item-name">
                            {team.team_name}
                          </span>
                          {selectedTeam?.id === team.id && (
                            <span className="ufaUFRSecondStageForm-checkmark">
                              ✓
                            </span>
                          )}
                        </li>
        ...
        <div className="ufaUFRSecondStageForm-section">
          ...
          >
            <button
              className={`ufaUFRSecondStageForm-toggle ${selectedPlayer ? "has-selection" : ""} ${isPlayerOpen ? "active" : ""}`}
            ...
            {playerError && (
              <div className="ufaUFRSecondStageForm-error">{playerError}</div>
            )}
                  ...
                  <div className="ufaUFRSecondStageForm-list-wrapper">
                    <ul className="ufaUFRSecondStageForm-list">
                      {players.map((player) => (
                        <li
                          key={player.id}
                          className={`ufaUFRSecondStageForm-item ${selectedPlayer?.id === player.id ? "selected" : ""}`}
        ...
        <div className="ufaUFRSecondStageForm-section">
          ...
          <div className="ufaUFRSecondStageForm-input-section">
            <div className="ufaUFRSecondStageForm-minimum-salaries">
              <div className="ufaUFRSecondStageForm-minimum-title">
                Минимальная сумма контракта по стажу (тыс.):
              </div>
              <div className="ufaUFRSecondStageForm-minimum-grid">
                {MINIMUM_SALARY_NOTES.map((item, index) => (
                  <div
                    key={index}
                    className="ufaUFRSecondStageForm-minimum-item"
                  >
                    <span className="ufaUFRSecondStageForm-minimum-experience">
                      {item.experience} год:
                    </span>
                    <span className="ufaUFRSecondStageForm-minimum-salary">
                      {item.salary}
                    </span>
                  </div>
                ))}
              </div>
              <div className="ufaUFRSecondStageForm-minimum-note">
                * Минимальная сумма зависит от стажа игрока в NBA
              </div>
            </div>

            <div className="ufaUFRSecondStageForm-input-wrapper">
              <input
                ref={contractInputRef}
                type="text"
                className="ufaUFRSecondStageForm-input"
              ...
              {contractAmount && (
                <button
                  className="ufaUFRSecondStageForm-input-clear"
            ...
            <div className="ufaUFRSecondStageForm-input-hint">
            ...
            {contractAmount &&
              selectedContractType &&
              selectedContractLength && (
                <div className="ufaUFRSecondStageForm-yearly-amounts">
                  <div className="ufaUFRSecondStageForm-yearly-title">
                    Сумма контракта по годам ({selectedContractType.label}):
                  </div>
                  <div className="ufaUFRSecondStageForm-yearly-list">
                    {yearlyContractAmounts.map((amount, index) => {
                      ...
                      return (
                        <div
                          key={index}
                          className="ufaUFRSecondStageForm-yearly-item"
                        >
                          <span className="ufaUFRSecondStageForm-year-label">
                            Год {index + 1}
                            {optionLabel && (
                              <span className="ufaUFRSecondStageForm-year-option">
                                {optionLabel}
                              </span>
                            )}
                          </span>
                          <span className="ufaUFRSecondStageForm-year-amount">
                            {formatAmount(amount)} тыс.
                          </span>
                          {index > 0 && (
                            <span className="ufaUFRSecondStageForm-year-change">
                              {selectedContractType.id === "flat" ? (
                                "без изменений"
                              ) : selectedContractType.id === "front" ? (
                                <span className="ufaUFRSecondStageForm-decrease">
                                  ▼ 5%
                                </span>
                              ) : selectedContractType.id === "back" ? (
                                <span className="ufaUFRSecondStageForm-increase">
                                  ▲ 5%
                                </span>
                              ) : (
                                <span className="ufaUFRSecondStageForm-increase">
                    ...
                    {yearlyContractAmounts.length > 0 && (
                      <div className="ufaUFRSecondStageForm-total-amount">
                        <span className="ufaUFRSecondStageForm-total-label">
                          Общая сумма:
                        </span>
                        <span className="ufaUFRSecondStageForm-total-value">
                          {formatAmount(
        ...
        {/* Тип контракта */}
        <div className="ufaUFRSecondStageForm-section">
          <h3 className="ufaUFRSecondStageForm-section-title">Тип контракта</h3>
          <div
            className="ufaUFRSecondStageForm-dropdown-wrapper"
            ref={contractTypeDropdownRef}
          >
            <button
              className={`ufaUFRSecondStageForm-toggle ${selectedContractType ? "has-selection" : ""} ${isContractTypeOpen ? "active" : ""}`}
                ...
                <div className="ufaUFRSecondStageForm-list-wrapper">
                  <ul className="ufaUFRSecondStageForm-list">
                    {CONTRACT_OPTIONS.map((option) => (
                      <li
                        key={option.id}
                        className={`ufaUFRSecondStageForm-item ${selectedContractType?.id === option.id ? "selected" : ""}`}
                        onClick={() => handleContractTypeSelect(option)}
                      >
                        <div className="ufaUFRSecondStageForm-contract-option">
                          <div className="ufaUFRSecondStageForm-contract-label">
                            {option.label}
                          </div>
                          <div className="ufaUFRSecondStageForm-contract-description">
                            {option.description}
                          </div>
                        </div>
        ...
        <div className="ufaUFRSecondStageForm-section">
          ...
          >
            <button
              className={`ufaUFRSecondStageForm-toggle ${selectedContractLength ? "has-selection" : ""} ${isContractLengthOpen ? "active" : ""}`}
                ...
                <div className="ufaUFRSecondStageForm-list-wrapper">
                  <ul className="ufaUFRSecondStageForm-list">
                    {CONTRACT_LENGTH_OPTIONS.map((option) => (
                      <li
                        key={option.id}
                        className={`ufaUFRSecondStageForm-item ${selectedContractLength?.id === option.id ? "selected" : ""}`}
        ...
        <div className="ufaUFRSecondStageForm-section">
          ...
          >
            <button
              className={`ufaUFRSecondStageForm-toggle ${selectedContractClause ? "has-selection" : ""} ${isContractClauseOpen ? "active" : ""}`}
                ...
                <div className="ufaUFRSecondStageForm-list-wrapper">
                  <ul className="ufaUFRSecondStageForm-list">
                    {CONTRACT_CLAUSE_OPTIONS.map((option) => (
                      <li
                        key={option.id}
                        className={`ufaUFRSecondStageForm-item ${selectedContractClause?.id === option.id ? "selected" : ""}`}
        ...
        <div className="ufaUFRSecondStageForm-section">
          ...
          >
            <button
              className={`ufaUFRSecondStageForm-toggle ${selectedSigningCondition ? "has-selection" : ""} ${isSigningConditionOpen ? "active" : ""}`}
                ...
                <div className="ufaUFRSecondStageForm-list-wrapper">
                  <ul className="ufaUFRSecondStageForm-list">
                    {SIGNING_CONDITION_OPTIONS.map((option) => (
                      <li
                        key={option.id}
                        className={`ufaUFRSecondStageForm-item ${selectedSigningCondition?.id === option.id ? "selected" : ""}`}
        ...
        {selectedSigningCondition?.id === "other" && (
          <div className="ufaUFRSecondStageForm-section">
            <h3 className="ufaUFRSecondStageForm-section-title">Комментарий</h3>
            <div className="ufaUFRSecondStageForm-input-section">
              <div className="ufaUFRSecondStageForm-input-wrapper">
                <input
                  ref={commentInputRef}
        ...
        {/* Кнопка отправки */}
        <div className="ufaUFRSecondStageForm-section ufaUFRSecondStageForm-submit-section">
          <button
            className="ufaUFRSecondStageForm-submit-button"
...
export default UFAUFRSecondStageForm;
