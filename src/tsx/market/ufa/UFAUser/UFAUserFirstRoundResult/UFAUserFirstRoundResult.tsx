...
type UFAFirstRoundResultRow = {
  player: string;
  ufa_first_round_first_stage_result?: string;
  ufa_first_round_second_stage_result?: string;
};

const UFAUserFirstRoundResult = () => {
  ...
  const fetchFirstStage = async () => {
    ...
    try {
      const { data, error } = (await supabase
        ...
        .select("player, ufa_first_round_first_stage_result")
        .eq("ufa_first_round_first_stage_result", selectedTeam)) as {
        data: UFAFirstRoundResultRow[] | null;
        error: any;
      };
      ...
      const filtered =
        data?.filter(
          (item) => item.ufa_first_round_first_stage_result === selectedTeam,
        ) ?? [];
      setPlayersFirstStage(filtered.map((item) => item.player));
    } catch (error) {
      console.error("Ошибка загрузки результатов 1 этапа:", error);
      setShowFirstStage(false);
    } finally {
  ...
  const fetchSecondStage = async () => {
    ...
    try {
      const { data, error } = (await supabase
        .from("Market_UFA_first_round_result")
        .select("player, ufa_first_round_second_stage_result")
        .eq("ufa_first_round_second_stage_result", selectedTeam)) as {
        data: UFAFirstRoundResultRow[] | null;
        error: any;
      };
      ...
      const filtered =
        data?.filter(
          (item) => item.ufa_first_round_second_stage_result === selectedTeam,
        ) ?? [];
      setPlayersSecondStage(filtered.map((item) => item.player));
    } catch (error) {
      console.error("Ошибка загрузки результатов 2 этапа:", error);
      setShowSecondStage(false);
    } finally {
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      >
        Выбор команды NBA — Результаты первого раунда UFA (два этапа)
      </h2>
            ...
            <div style={{ marginTop: "10px" }}>
              ...
              {passwordVerified === true && (
                ...
                >
                  ...
                  {/* Блок результатов первого этапа */}
                  {showFirstStage && (
                    <div style={{ marginBottom: "20px" }}>
                      <h3 style={{ marginBottom: "10px", fontWeight: "bold" }}>
                        Результаты 1 этапа первого раунда UFA для команды{" "}
                        {selectedTeam}:
                      </h3>
                      <div
                        style={{
                        ...
                        {loadingFirstStage ? (
                          ...
                          <div>
                            <p style={{ marginBottom: "8px" }}>
                              Победы в первом этапе первого раунда UFA по
                              следующим игрокам:
                            </p>
                          ...
                          <p>
                            Ваша команда не выиграла в 1 этапе первого раунда
                  ...
                  {/* Блок результатов второго этапа */}
                  {showSecondStage && (
                    <div>
                      <h3 style={{ marginBottom: "10px", fontWeight: "bold" }}>
                        Результаты 2 этапа первого раунда UFA для команды{" "}
                        {selectedTeam}:
                      </h3>
                      <div
                        style={{
                      ...
                      >
                        {loadingSecondStage ? (
                          ...
                          <div>
                            <p style={{ marginBottom: "8px" }}>
                              Победы во втором этапе первого раунда UFA по
                              следующим игрокам:
                            </p>
                            <ul style={{ listStyleType: "none", padding: 0 }}>
                          ...
                          <p>
                            Ваша команда не выиграла во 2 этапе первого раунда
                            UFA или результаты ещё не внесены
                          </p>
                        )}
                      </div>
...
export default UFAUserFirstRoundResult;
