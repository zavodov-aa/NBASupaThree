...
import "./ufaUserSecondRoundResult.css"; // можно оставить, если есть файл, но стили инлайн

type UFASecondRoundResultRow = {
  player: string;
  ufa_second_round_first_stage_result?: string;
  ufa_second_round_second_stage_result?: string;
};

const UFAUserSecondRoundResult = () => {
  ...
  // Состояния для первого этапа второго раунда
  ...
  // Состояния для второго этапа второго раунда
  ...
  const fetchFirstStage = async () => {
    ...
    try {
      const { data, error } = (await supabase
        ...
        .select("player, ufa_second_round_first_stage_result")
        .eq("ufa_second_round_first_stage_result", selectedTeam)) as {
        data: UFASecondRoundResultRow[] | null;
        error: any;
      };
      ...
      const filtered =
        data?.filter(
          (item) => item.ufa_second_round_first_stage_result === selectedTeam,
        ) ?? [];
      setPlayersFirstStage(filtered.map((item) => item.player));
    } catch (error) {
      console.error(
        "Ошибка загрузки результатов 1 этапа второго раунда:",
        error,
      );
      setShowFirstStage(false);
    } finally {
  ...
  const fetchSecondStage = async () => {
    ...
    try {
      const { data, error } = (await supabase
        .from("Market_UFA_second_round_result")
        .select("player, ufa_second_round_second_stage_result")
        .eq("ufa_second_round_second_stage_result", selectedTeam)) as {
        data: UFASecondRoundResultRow[] | null;
        error: any;
      };
      ...
      const filtered =
        data?.filter(
          (item) => item.ufa_second_round_second_stage_result === selectedTeam,
        ) ?? [];
      setPlayersSecondStage(filtered.map((item) => item.player));
    } catch (error) {
      console.error(
        "Ошибка загрузки результатов 2 этапа второго раунда:",
        error,
      );
      setShowSecondStage(false);
    } finally {
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      >
        Выбор команды NBA — Результаты второго раунда UFA (два этапа)
      </h2>
            ...
            <div style={{ marginTop: "10px" }}>
              ...
              {passwordVerified === true && (
                ...
                >
                  ...
                  {/* Две кнопки для этапов второго раунда */}
                  <div
                    style={{
                      display: "flex",
                      gap: "10px",
                  ...
                  {/* Блок результатов первого этапа второго раунда */}
                  {showFirstStage && (
                    <div style={{ marginBottom: "20px" }}>
                      <h3 style={{ marginBottom: "10px", fontWeight: "bold" }}>
                        Результаты 1 этапа второго раунда UFA для команды{" "}
                        {selectedTeam}:
                      </h3>
                      <div
                        style={{
                        ...
                        {loadingFirstStage ? (
                          ...
                          <div>
                            <p style={{ marginBottom: "8px" }}>
                              Победы в первом этапе второго раунда UFA по
                              следующим игрокам:
                            </p>
                          ...
                          <p>
                            Ваша команда не выиграла в 1 этапе второго раунда
                  ...
                  {/* Блок результатов второго этапа второго раунда */}
                  {showSecondStage && (
                    <div>
                      <h3 style={{ marginBottom: "10px", fontWeight: "bold" }}>
                        Результаты 2 этапа второго раунда UFA для команды{" "}
                        {selectedTeam}:
                      </h3>
                      <div
                        style={{
                      ...
                      >
                        {loadingSecondStage ? (
                          ...
                          <div>
                            <p style={{ marginBottom: "8px" }}>
                              Победы во втором этапе второго раунда UFA по
                              следующим игрокам:
                            </p>
                            <ul style={{ listStyleType: "none", padding: 0 }}>
                          ...
                          <p>
                            Ваша команда не выиграла во 2 этапе второго раунда
                            UFA или результаты ещё не внесены
                          </p>
                        )}
                      </div>
...
export default UFAUserSecondRoundResult;
