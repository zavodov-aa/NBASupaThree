...
import "./ufaUTRFirstStageForm.css";
...
const UFAUTRFirstStageForm: React.FC = () => {
  ...
  const handleSubmit = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      // ИЗМЕНЕНО: таблица для третьего раунда
      const { error } = await supabase
        .from("Market_UFA_third_round_first_stage")
        .insert([dataToInsert]);
      ...
      console.log(
        "Данные успешно сохранены в таблицу Market_UFA_third_round_first_stage",
      );
  ...
  return (
    <div className="ufaUTRFirstStageForm-container">
      <div className="ufaUTRFirstStageForm-content">
        <div className="ufaUTRFirstStageForm-section-header">
          <div className="ufaUTRFirstStageForm-title-wrapper">
            <h2 className="ufaUTRFirstStageForm-main-title">
              СОЗДАНИЕ ПРЕДЛОЖЕНИЯ UFA (ТРЕТИЙ РАУНД, ПЕРВЫЙ ЭТАП)
            </h2>
            <p className="ufaUTRFirstStageForm-subtitle">
              Заполните все поля для подачи предложения
            </p>
            <div className="ufaUTRFirstStageForm-divider"></div>
          </div>
        </div>

        {submitError && (
          <div className="ufaUTRFirstStageForm-error-message">
            {submitError}
          </div>
        )}

        {submitSuccess && (
          <div className="ufaUTRFirstStageForm-success-message">
            Предложение успешно отправлено!
          </div>
        )}

        <div className="ufaUTRFirstStageForm-section">
          <h3 className="ufaUTRFirstStageForm-section-title">
            Выберите команду NBA
          </h3>
          <div
            className="ufaUTRFirstStageForm-dropdown-wrapper"
            ref={teamDropdownRef}
          >
            <button
              className={`ufaUTRFirstStageForm-toggle ${selectedTeam ? "has-selection" : ""} ${isTeamOpen ? "active" : ""}`}
              onClick={toggleTeamDropdown}
              disabled={loadingTeams}
              type="button"
            >
              <span className="ufaUTRFirstStageForm-button-text">
                {getTeamButtonText()}
              </span>
              {selectedTeam && (
                <button
                  className="ufaUTRFirstStageForm-clear"
              ...
              <span className="ufaUTRFirstStageForm-arrow">
                {isTeamOpen ? "▲" : "▼"}
              </span>
            </button>

            {teamError && (
              <div className="ufaUTRFirstStageForm-error">{teamError}</div>
            )}

            {isTeamOpen && (
              <div className="ufaUTRFirstStageForm-menu">
                {loadingTeams ? (
                  <div className="ufaUTRFirstStageForm-loading">
                    <div className="ufaUTRFirstStageForm-spinner"></div>
                    <span>Загрузка команд...</span>
                  </div>
                ) : teams.length === 0 ? (
                  <div className="ufaUTRFirstStageForm-empty">
                    Команды не найдены
                  </div>
                ) : (
                  <div className="ufaUTRFirstStageForm-list-wrapper">
                    <ul className="ufaUTRFirstStageForm-list">
                      {teams.map((team) => (
                        <li
                          key={team.id}
                          className={`ufaUTRFirstStageForm-item ${selectedTeam?.id === team.id ? "selected" : ""}`}
                          onClick={() => handleTeamSelect(team)}
                        >
                          <span className="ufaUTRFirstStageForm-item-name">
                            {team.team_name}
                          </span>
                          {selectedTeam?.id === team.id && (
                            <span className="ufaUTRFirstStageForm-checkmark">
                              ✓
                            </span>
                          )}
                        </li>
        ...
        <div className="ufaUTRFirstStageForm-section">
          ...
          >
            <button
              className={`ufaUTRFirstStageForm-toggle ${selectedPlayer ? "has-selection" : ""} ${isPlayerOpen ? "active" : ""}`}
            ...
            {playerError && (
              <div className="ufaUTRFirstStageForm-error">{playerError}</div>
            )}
                  ...
                  <div className="ufaUTRFirstStageForm-list-wrapper">
                    <ul className="ufaUTRFirstStageForm-list">
                      {players.map((player) => (
                        <li
                          key={player.id}
                          className={`ufaUTRFirstStageForm-item ${selectedPlayer?.id === player.id ? "selected" : ""}`}
        ...
        <div className="ufaUTRFirstStageForm-section">
          ...
          <div className="ufaUTRFirstStageForm-input-section">
            <div className="ufaUTRFirstStageForm-minimum-salaries">
              <div className="ufaUTRFirstStageForm-minimum-title">
                Минимальная сумма контракта по стажу (тыс.):
              </div>
              <div className="ufaUTRFirstStageForm-minimum-grid">
                {MINIMUM_SALARY_NOTES.map((item, index) => (
                  <div
                    key={index}
                    className="ufaUTRFirstStageForm-minimum-item"
                  >
                    <span className="ufaUTRFirstStageForm-minimum-experience">
                      {item.experience} год:
                    </span>
                    <span className="ufaUTRFirstStageForm-minimum-salary">
                      {item.salary}
                    </span>
                  </div>
                ))}
              </div>
              <div className="ufaUTRFirstStageForm-minimum-note">
                * Минимальная сумма зависит от стажа игрока в NBA
              </div>
            </div>

            <div className="ufaUTRFirstStageForm-input-wrapper">
              <input
                ref={contractInputRef}
                type="text"
                className="ufaUTRFirstStageForm-input"
              ...
              {contractAmount && (
                <button
                  className="ufaUTRFirstStageForm-input-clear"
            ...
            <div className="ufaUTRFirstStageForm-input-hint">
            ...
            {contractAmount &&
              selectedContractType &&
              selectedContractLength && (
                <div className="ufaUTRFirstStageForm-yearly-amounts">
                  <div className="ufaUTRFirstStageForm-yearly-title">
                    Сумма контракта по годам ({selectedContractType.label}):
                  </div>
                  <div className="ufaUTRFirstStageForm-yearly-list">
                    {yearlyContractAmounts.map((amount, index) => {
                      ...
                      return (
                        <div
                          key={index}
                          className="ufaUTRFirstStageForm-yearly-item"
                        >
                          <span className="ufaUTRFirstStageForm-year-label">
                            Год {index + 1}
                            {optionLabel && (
                              <span className="ufaUTRFirstStageForm-year-option">
                                {optionLabel}
                              </span>
                            )}
                          </span>
                          <span className="ufaUTRFirstStageForm-year-amount">
                            {formatAmount(amount)} тыс.
                          </span>
                          {index > 0 && (
                            <span className="ufaUTRFirstStageForm-year-change">
                              {selectedContractType.id === "flat" ? (
                                "без изменений"
                              ) : selectedContractType.id === "front" ? (
                                <span className="ufaUTRFirstStageForm-decrease">
                                  ▼ 5%
                                </span>
                              ) : selectedContractType.id === "back" ? (
                                <span className="ufaUTRFirstStageForm-increase">
                                  ▲ 5%
                                </span>
                              ) : (
                                <span className="ufaUTRFirstStageForm-increase">
                    ...
                    {yearlyContractAmounts.length > 0 && (
                      <div className="ufaUTRFirstStageForm-total-amount">
                        <span className="ufaUTRFirstStageForm-total-label">
                          Общая сумма:
                        </span>
                        <span className="ufaUTRFirstStageForm-total-value">
                          {formatAmount(
        ...
        <div className="ufaUTRFirstStageForm-section">
          <h3 className="ufaUTRFirstStageForm-section-title">Тип контракта</h3>
          <div
            className="ufaUTRFirstStageForm-dropdown-wrapper"
            ref={contractTypeDropdownRef}
          >
            <button
              className={`ufaUTRFirstStageForm-toggle ${selectedContractType ? "has-selection" : ""} ${isContractTypeOpen ? "active" : ""}`}
                ...
                <div className="ufaUTRFirstStageForm-list-wrapper">
                  <ul className="ufaUTRFirstStageForm-list">
                    {CONTRACT_OPTIONS.map((option) => (
                      <li
                        key={option.id}
                        className={`ufaUTRFirstStageForm-item ${selectedContractType?.id === option.id ? "selected" : ""}`}
                        onClick={() => handleContractTypeSelect(option)}
                      >
                        <div className="ufaUTRFirstStageForm-contract-option">
                          <div className="ufaUTRFirstStageForm-contract-label">
                            {option.label}
                          </div>
                          <div className="ufaUTRFirstStageForm-contract-description">
                            {option.description}
                          </div>
                        </div>
        ...
        <div className="ufaUTRFirstStageForm-section">
          <h3 className="ufaUTRFirstStageForm-section-title">Срок контракта</h3>
          <div
            className="ufaUTRFirstStageForm-dropdown-wrapper"
            ref={contractLengthDropdownRef}
          >
            <button
              className={`ufaUTRFirstStageForm-toggle ${selectedContractLength ? "has-selection" : ""} ${isContractLengthOpen ? "active" : ""}`}
                ...
                <div className="ufaUTRFirstStageForm-list-wrapper">
                  <ul className="ufaUTRFirstStageForm-list">
                    {CONTRACT_LENGTH_OPTIONS.map((option) => (
                      <li
                        key={option.id}
                        className={`ufaUTRFirstStageForm-item ${selectedContractLength?.id === option.id ? "selected" : ""}`}
        ...
        <div className="ufaUTRFirstStageForm-section">
          ...
          >
            <button
              className={`ufaUTRFirstStageForm-toggle ${selectedContractClause ? "has-selection" : ""} ${isContractClauseOpen ? "active" : ""}`}
                ...
                <div className="ufaUTRFirstStageForm-list-wrapper">
                  <ul className="ufaUTRFirstStageForm-list">
                    {CONTRACT_CLAUSE_OPTIONS.map((option) => (
                      <li
                        key={option.id}
                        className={`ufaUTRFirstStageForm-item ${selectedContractClause?.id === option.id ? "selected" : ""}`}
        ...
        <div className="ufaUTRFirstStageForm-section">
          ...
          >
            <button
              className={`ufaUTRFirstStageForm-toggle ${selectedSigningCondition ? "has-selection" : ""} ${isSigningConditionOpen ? "active" : ""}`}
                ...
                <div className="ufaUTRFirstStageForm-list-wrapper">
                  <ul className="ufaUTRFirstStageForm-list">
                    {SIGNING_CONDITION_OPTIONS.map((option) => (
                      <li
                        key={option.id}
                        className={`ufaUTRFirstStageForm-item ${selectedSigningCondition?.id === option.id ? "selected" : ""}`}
        ...
        {selectedSigningCondition?.id === "other" && (
          <div className="ufaUTRFirstStageForm-section">
            <h3 className="ufaUTRFirstStageForm-section-title">Комментарий</h3>
            <div className="ufaUTRFirstStageForm-input-section">
              <div className="ufaUTRFirstStageForm-input-wrapper">
                <input
                  ref={commentInputRef}
        ...
        <div className="ufaUTRFirstStageForm-section ufaUTRFirstStageForm-submit-section">
          <button
            className="ufaUTRFirstStageForm-submit-button"
...
export default UFAUTRFirstStageForm;
