...
const RosterSevSix: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr
        key={player.id}
        className={isFranchisePlayer ? "franchiseSevSix" : ""}
      >
        <td>
          <div className="number-cellSevSix">
            {index + 1}
            {isFranchisePlayer && <span className="starSevSix">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagSevSix">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightSevSix">
  ...
  return (
    <div className="rosterSevSix">
      <div className="roster-headerSevSix">
        <h1>PHILADELPHIA 76ERS ROSTER</h1>
        <div className="roster-subtitleSevSix">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerSevSix">
          <table className="roster-tableSevSix">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerSevSix">
                  <td colSpan={14}>
                    <div className="g-league-headerSevSix">G-LEAGUE ROSTER</div>
                  </td>
                </tr>
              )}
        ...
        <div className="empty-stateSevSix">
          <div className="empty-iconSevSix">🏀</div>
          <div className="empty-titleSevSix">No Players in Roster</div>
          <div className="empty-subtitleSevSix">
            Currently no players are assigned to the Philadelphia 76ers team
          </div>
        </div>
      )}
    </div>
