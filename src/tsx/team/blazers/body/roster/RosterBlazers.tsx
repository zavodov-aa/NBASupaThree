    ...
    const fetchPlayers = async () => {
      try {
        const { data, error } = await supabase
          .from("Players")
          .select("*")
          .or("id.eq.1,team.eq.Portland Trail Blazers")
          .order("id");
    ...
    <div className="rosterBlazers">
        ...
        <table className="roster-tableBlazers">
          <tbody>
            {players.map((player, index) => (
              <tr
                key={player.id}
                className={player.id === 1 ? "franchiseBlazers" : ""}
              >
                <td>
                  <div className="number-cellBlazers">
                    {index + 0}
                    {player.id === 1 && <span className="starBlazers">★</span>}
                  </div>
                </td>
                <td>
                  <span className="player-tagBlazers ">
                ...
                <td>{player.age || "—"}</td>
                <td className="highlightBlazers">{player.year_one || "—"}</td>
      ...
      {players.length === 0 && (
        <div className="empty-stateBlazers">
          <div className="empty-iconBlazers">🏀</div>
