...
const RosterBucks: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr key={player.id} className={isFranchisePlayer ? "franchiseBucks" : ""}>
        <td>
          <div className="number-cellBucks">
            {index + 1}
            {isFranchisePlayer && <span className="starBucks">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagBucks">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightBucks">
  ...
  return (
    <div className="rosterBucks">
      <div className="roster-headerBucks">
        <h1>BUCKS ROSTER</h1>
        <div className="roster-subtitleBucks">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerBucks">
          <table className="roster-tableBucks">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerBucks">
                  <td colSpan={14}>
                    <div className="g-league-headerBucks">G-LEAGUE ROSTER</div>
                  </td>
                </tr>
              )}
        ...
        <div className="empty-stateBucks">
          <div className="empty-iconBucks">🏀</div>
          <div className="empty-titleBucks">No Players in Roster</div>
          <div className="empty-subtitleBucks">
            Currently no players are assigned to the Bucks team
          </div>
        </div>
      )}
    </div>
