...
const RosterBulls: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr key={player.id} className={isFranchisePlayer ? "franchiseBulls" : ""}>
        <td>
          <div className="number-cellBulls">
            {index + 1}
            {isFranchisePlayer && <span className="starBulls">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagBulls">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightBulls">
  ...
  return (
    <div className="rosterBulls">
      <div className="roster-headerBulls">
        <h1>BULLS ROSTER</h1>
        <div className="roster-subtitleBulls">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerBulls">
          <table className="roster-tableBulls">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerBulls">
                  <td colSpan={14}>
                    <div className="g-league-headerBulls">G-LEAGUE ROSTER</div>
                  </td>
                </tr>
              )}
        ...
        <div className="empty-stateBulls">
          <div className="empty-iconBulls">🏀</div>
          <div className="empty-titleBulls">No Players in Roster</div>
          <div className="empty-subtitleBulls">
            Currently no players are assigned to the Bulls team
          </div>
        </div>
      )}
    </div>
