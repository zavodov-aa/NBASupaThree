...
const RosterCavaliers: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr key={player.id} className={isFranchisePlayer ? "franchiseCavaliers" : ""}>
        <td>
          <div className="number-cellCavaliers">
            {index + 1}
            {isFranchisePlayer && <span className="starCavaliers">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagCavaliers">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightCavaliers">
  ...
  return (
    <div className="rosterCavaliers">
      <div className="roster-headerCavaliers">
        <h1>CAVALIERS ROSTER</h1>
        <div className="roster-subtitleCavaliers">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerCavaliers">
          <table className="roster-tableCavaliers">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerCavaliers">
                  <td colSpan={14}>
                    <div className="g-league-headerCavaliers">G-LEAGUE ROSTER</div>
                  </td>
                </tr>
              )}
        ...
        <div className="empty-stateCavaliers">
          <div className="empty-iconCavaliers">🏀</div>
          <div className="empty-titleCavaliers">No Players in Roster</div>
          <div className="empty-subtitleCavaliers">
            Currently no players are assigned to the Cavaliers team
          </div>
        </div>
      )}
    </div>
...
export default RosterCavaliers;