...
const RosterCeltics: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr
        key={player.id}
        className={isFranchisePlayer ? "franchiseCeltics" : ""}
      >
        <td>
          <div className="number-cellCeltics">
            {index + 1}
            {isFranchisePlayer && <span className="starCeltics">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagCeltics">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightCeltics">
  ...
  return (
    <div className="rosterCeltics">
      <div className="roster-headerCeltics">
        <h1>CELTICS ROSTER</h1>
        <div className="roster-subtitleCeltics">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerCeltics">
          <table className="roster-tableCeltics">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerCeltics">
                  <td colSpan={14}>
                    <div className="g-league-headerCeltics">
                      G-LEAGUE ROSTER
                    </div>
                  </td>
                </tr>
        ...
        <div className="empty-stateCeltics">
          <div className="empty-iconCeltics">🏀</div>
          <div className="empty-titleCeltics">No Players in Roster</div>
          <div className="empty-subtitleCeltics">
            Currently no players are assigned to the Celtics team
          </div>
        </div>
      )}
    </div>
