...
const RosterClippers: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr
        key={player.id}
        className={isFranchisePlayer ? "franchiseClippers" : ""}
      >
        <td>
          <div className="number-cellClippers">
            {index + 1}
            {isFranchisePlayer && <span className="starClippers">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagClippers">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightClippers">
  ...
  return (
    <div className="rosterClippers">
      <div className="roster-headerClippers">
        <h1>CLIPPERS ROSTER</h1>
        <div className="roster-subtitleClippers">
          ACTIVE PLAYERS & CONTRACTS
        </div>
      </div>

      {hasPlayers ? (
        <div className="table-containerClippers">
          <table className="roster-tableClippers">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerClippers">
                  <td colSpan={14}>
                    <div className="g-league-headerClippers">
                      G-LEAGUE ROSTER
                    </div>
                  </td>
                </tr>
        ...
        <div className="empty-stateClippers">
          <div className="empty-iconClippers">🏀</div>
          <div className="empty-titleClippers">No Players in Roster</div>
          <div className="empty-subtitleClippers">
            Currently no players are assigned to the Clippers team
          </div>
        </div>
      )}
    </div>
