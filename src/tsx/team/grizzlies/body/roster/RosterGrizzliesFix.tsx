...
const RosterGrizzlies: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr
        key={player.id}
        className={isFranchisePlayer ? "franchiseGrizzlies" : ""}
      >
        <td>
          <div className="number-cellGrizzlies">
            {index + 1}
            {isFranchisePlayer && <span className="starGrizzlies">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagGrizzlies">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightGrizzlies">
  ...
  return (
    <div className="rosterGrizzlies">
      <div className="roster-headerGrizzlies">
        <h1>GRIZZLIES ROSTER</h1>
        <div className="roster-subtitleGrizzlies">
          ACTIVE PLAYERS & CONTRACTS
        </div>
      </div>

      {hasPlayers ? (
        <div className="table-containerGrizzlies">
          <table className="roster-tableGrizzlies">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerGrizzlies">
                  <td colSpan={14}>
                    <div className="g-league-headerGrizzlies">
                      G-LEAGUE ROSTER
                    </div>
                  </td>
                </tr>
        ...
        <div className="empty-stateGrizzlies">
          <div className="empty-iconGrizzlies">🏀</div>
          <div className="empty-titleGrizzlies">No Players in Roster</div>
          <div className="empty-subtitleGrizzlies">
            Currently no players are assigned to the Grizzlies team
          </div>
        </div>
      )}
    </div>
...
export default RosterGrizzlies;
