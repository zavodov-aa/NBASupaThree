...
const RosterHawks: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr key={player.id} className={isFranchisePlayer ? "franchiseHawks" : ""}>
        <td>
          <div className="number-cellHawks">
            {index + 1}
            {isFranchisePlayer && <span className="starHawks">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagHawks">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightHawks">
  ...
  return (
    <div className="rosterHawks">
      <div className="roster-headerHawks">
        <h1>HAWKS ROSTER</h1>
        <div className="roster-subtitleHawks">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerHawks">
          <table className="roster-tableHawks">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerHawks">
                  <td colSpan={14}>
                    <div className="g-league-headerHawks">G-LEAGUE ROSTER</div>
                  </td>
                </tr>
              )}
        ...
        <div className="empty-stateHawks">
          <div className="empty-iconHawks">🏀</div>
          <div className="empty-titleHawks">No Players in Roster</div>
          <div className="empty-subtitleHawks">
            Currently no players are assigned to the Hawks team
          </div>
        </div>
      )}
    </div>
