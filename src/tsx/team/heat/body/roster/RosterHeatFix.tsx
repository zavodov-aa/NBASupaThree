...
const RosterHeat: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr key={player.id} className={isFranchisePlayer ? "franchiseHeat" : ""}>
        <td>
          <div className="number-cellHeat">
            {index + 1}
            {isFranchisePlayer && <span className="starHeat">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagHeat">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightHeat">
  ...
  return (
    <div className="rosterHeat">
      <div className="roster-headerHeat">
        <h1>HEAT ROSTER</h1>
        <div className="roster-subtitleHeat">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerHeat">
          <table className="roster-tableHeat">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerHeat">
                  <td colSpan={14}>
                    <div className="g-league-headerHeat">G-LEAGUE ROSTER</div>
                  </td>
                </tr>
              )}
        ...
        <div className="empty-stateHeat">
          <div className="empty-iconHeat">🏀</div>
          <div className="empty-titleHeat">No Players in Roster</div>
          <div className="empty-subtitleHeat">
            Currently no players are assigned to the Heat team
          </div>
        </div>
      )}
    </div>
