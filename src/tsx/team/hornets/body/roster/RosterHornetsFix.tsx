...
const RosterHornets: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr
        key={player.id}
        className={isFranchisePlayer ? "franchiseHornets" : ""}
      >
        <td>
          <div className="number-cellHornets">
            {index + 1}
            {isFranchisePlayer && <span className="starHornets">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagHornets">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightHornets">
  ...
  return (
    <div className="rosterHornets">
      <div className="roster-headerHornets">
        <h1>HORNETS ROSTER</h1>
        <div className="roster-subtitleHornets">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerHornets">
          <table className="roster-tableHornets">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerHornets">
                  <td colSpan={14}>
                    <div className="g-league-headerHornets">
                      G-LEAGUE ROSTER
                    </div>
                  </td>
                </tr>
        ...
        <div className="empty-stateHornets">
          <div className="empty-iconHornets">🏀</div>
          <div className="empty-titleHornets">No Players in Roster</div>
          <div className="empty-subtitleHornets">
            Currently no players are assigned to the Hornets team
          </div>
        </div>
      )}
    </div>
