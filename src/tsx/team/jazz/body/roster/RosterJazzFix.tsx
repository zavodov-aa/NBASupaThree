...
const RosterJazz: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr key={player.id} className={isFranchisePlayer ? "franchiseJazz" : ""}>
        <td>
          <div className="number-cellJazz">
            {index + 1}
            {isFranchisePlayer && <span className="starJazz">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagJazz">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightJazz">
  ...
  return (
    <div className="rosterJazz">
      <div className="roster-headerJazz">
        <h1>JAZZ ROSTER</h1>
        <div className="roster-subtitleJazz">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerJazz">
          <table className="roster-tableJazz">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerJazz">
                  <td colSpan={14}>
                    <div className="g-league-headerJazz">G-LEAGUE ROSTER</div>
                  </td>
                </tr>
              )}
        ...
        <div className="empty-stateJazz">
          <div className="empty-iconJazz">🏀</div>
          <div className="empty-titleJazz">No Players in Roster</div>
          <div className="empty-subtitleJazz">
            Currently no players are assigned to the Jazz team
          </div>
        </div>
      )}
    </div>
