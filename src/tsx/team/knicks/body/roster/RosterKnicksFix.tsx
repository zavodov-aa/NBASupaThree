...
const RosterKnicks: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr
        key={player.id}
        className={isFranchisePlayer ? "franchiseKnicks" : ""}
      >
        <td>
          <div className="number-cellKnicks">
            {index + 1}
            {isFranchisePlayer && <span className="starKnicks">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagKnicks">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightKnicks">
  ...
  return (
    <div className="rosterKnicks">
      <div className="roster-headerKnicks">
        <h1>KNICKS ROSTER</h1>
        <div className="roster-subtitleKnicks">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerKnicks">
          <table className="roster-tableKnicks">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerKnicks">
                  <td colSpan={14}>
                    <div className="g-league-headerKnicks">G-LEAGUE ROSTER</div>
                  </td>
                </tr>
              )}
        ...
        <div className="empty-stateKnicks">
          <div className="empty-iconKnicks">🏀</div>
          <div className="empty-titleKnicks">No Players in Roster</div>
          <div className="empty-subtitleKnicks">
            Currently no players are assigned to the Knicks team
          </div>
        </div>
      )}
    </div>
