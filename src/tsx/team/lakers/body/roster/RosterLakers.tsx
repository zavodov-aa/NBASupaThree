  ...
  return (
    <div className="roster">
      <div className="roster-header">
        <h1>LOS ANGELES LAKERS ROSTER</h1>
        <div className="roster-subtitle">ACTIVE PLAYERS & CONTRACTS</div>
      </div>
        ...
        <table className="roster-table">
          <tbody>
            {players.map((player, index) => (
              <tr
                key={player.id}
                className={player.id === 1 ? "franchise" : ""}
              >
                <td>
                  <div className="number-cell">
                    {index + 0}
                    {player.id === 1 && <span className="star">★</span>}
                  </div>
                </td>
                <td>
                  <span className="player-tag ">{player.active_roster}</span>
                </td>
                <td>{player.position}</td>
                <td>{player.pos_elig}</td>
                <td>{player.age || "—"}</td>
                <td className="highlight">{player.year_one || "—"}</td>
      ...
      {players.length === 0 && (
        <div className="empty-state">
          ...
          <div className="empty-title">No Players in Roster</div>
