...
const RosterLakers: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr
        key={player.id}
        className={isFranchisePlayer ? "franchiseLakers" : ""}
      >
        <td>
          <div className="number-cellLakers">
            {index + 1}
            {isFranchisePlayer && <span className="starLakers">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagLakers">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightLakers">
  ...
  return (
    <div className="rosterLakers">
      <div className="roster-headerLakers">
        <h1>LAKERS ROSTER</h1>
        <div className="roster-subtitleLakers">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerLakers">
          <table className="roster-tableLakers">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerLakers">
                  <td colSpan={14}>
                    <div className="g-league-headerLakers">G-LEAGUE ROSTER</div>
                  </td>
                </tr>
              )}
        ...
        <div className="empty-stateLakers">
          <div className="empty-iconLakers">🏀</div>
          <div className="empty-titleLakers">No Players in Roster</div>
          <div className="empty-subtitleLakers">
            Currently no players are assigned to the Lakers team
          </div>
        </div>
      )}
    </div>
