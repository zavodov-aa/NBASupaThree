...
const RosterMagic: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr key={player.id} className={isFranchisePlayer ? "franchiseMagic" : ""}>
        <td>
          <div className="number-cellMagic">
            {index + 1}
            {isFranchisePlayer && <span className="starMagic">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagMagic">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightMagic">
  ...
  return (
    <div className="rosterMagic">
      <div className="roster-headerMagic">
        <h1>MAGIC ROSTER</h1>
        <div className="roster-subtitleMagic">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerMagic">
          <table className="roster-tableMagic">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerMagic">
                  <td colSpan={14}>
                    <div className="g-league-headerMagic">G-LEAGUE ROSTER</div>
                  </td>
                </tr>
              )}
        ...
        <div className="empty-stateMagic">
          <div className="empty-iconMagic">🏀</div>
          <div className="empty-titleMagic">No Players in Roster</div>
          <div className="empty-subtitleMagic">
            Currently no players are assigned to the Magic team
          </div>
        </div>
      )}
    </div>
