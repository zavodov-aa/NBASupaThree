...
const RosterMavericks: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr
        key={player.id}
        className={isFranchisePlayer ? "franchiseMavericks" : ""}
      >
        <td>
          <div className="number-cellMavericks">
            {index + 1}
            {isFranchisePlayer && <span className="starMavericks">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagMavericks">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightMavericks">
  ...
  return (
    <div className="rosterMavericks">
      <div className="roster-headerMavericks">
        <h1>MAVERICKS ROSTER</h1>
        <div className="roster-subtitleMavericks">
          ACTIVE PLAYERS & CONTRACTS
        </div>
      </div>

      {hasPlayers ? (
        <div className="table-containerMavericks">
          <table className="roster-tableMavericks">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerMavericks">
                  <td colSpan={14}>
                    <div className="g-league-headerMavericks">
                      G-LEAGUE ROSTER
                    </div>
                  </td>
                </tr>
        ...
        <div className="empty-stateMavericks">
          <div className="empty-iconMavericks">🏀</div>
          <div className="empty-titleMavericks">No Players in Roster</div>
          <div className="empty-subtitleMavericks">
            Currently no players are assigned to the Mavericks team
          </div>
        </div>
      )}
    </div>
...
export default RosterMavericks;
