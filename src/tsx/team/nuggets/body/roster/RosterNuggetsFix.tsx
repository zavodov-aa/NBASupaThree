...
const RosterNuggets: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr
        key={player.id}
        className={isFranchisePlayer ? "franchiseNuggets" : ""}
      >
        <td>
          <div className="number-cellNuggets">
            {index + 1}
            {isFranchisePlayer && <span className="starNuggets">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagNuggets">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightNuggets">
  ...
  return (
    <div className="rosterNuggets">
      <div className="roster-headerNuggets">
        <h1>NUGGETS ROSTER</h1>
        <div className="roster-subtitleNuggets">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerNuggets">
          <table className="roster-tableNuggets">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerNuggets">
                  <td colSpan={14}>
                    <div className="g-league-headerNuggets">
                      G-LEAGUE ROSTER
                    </div>
                  </td>
                </tr>
        ...
        <div className="empty-stateNuggets">
          <div className="empty-iconNuggets">🏀</div>
          <div className="empty-titleNuggets">No Players in Roster</div>
          <div className="empty-subtitleNuggets">
            Currently no players are assigned to the Nuggets team
          </div>
        </div>
      )}
    </div>
