...
const RosterPacers: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr
        key={player.id}
        className={isFranchisePlayer ? "franchisePacers" : ""}
      >
        <td>
          <div className="number-cellPacers">
            {index + 1}
            {isFranchisePlayer && <span className="starPacers">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagPacers">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightPacers">
          ...
          <table className="roster-tablePacers">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerPacers">
                  <td colSpan={14}>
                    <div className="g-league-headerPacers">G-LEAGUE ROSTER</div>
                  </td>
                </tr>
              )}
