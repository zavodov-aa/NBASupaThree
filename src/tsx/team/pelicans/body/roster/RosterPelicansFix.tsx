...
const RosterPelicans: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr
        key={player.id}
        className={isFranchisePlayer ? "franchisePelicans" : ""}
      >
        <td>
          <div className="number-cellPelicans">
            {index + 1}
            {isFranchisePlayer && <span className="starPelicans">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagPelicans">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightPelicans">
          ...
          <table className="roster-tablePelicans">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerPelicans">
                  <td colSpan={14}>
                    <div className="g-league-headerPelicans">
                      G-LEAGUE ROSTER
                    </div>
                  </td>
                </tr>
