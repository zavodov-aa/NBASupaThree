...
const RosterPistons: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr
        key={player.id}
        className={isFranchisePlayer ? "franchisePistons" : ""}
      >
        <td>
          <div className="number-cellPistons">
            {index + 1}
            {isFranchisePlayer && <span className="starPistons">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagPistons">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightPistons">
  ...
  return (
    <div className="rosterPistons">
      <div className="roster-headerPistons">
        <h1>PISTONS ROSTER</h1>
        <div className="roster-subtitlePistons">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerPistons">
          <table className="roster-tablePistons">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerPistons">
                  <td colSpan={14}>
                    <div className="g-league-headerPistons">
                      G-LEAGUE ROSTER
                    </div>
                  </td>
                </tr>
        ...
        <div className="empty-statePistons">
          <div className="empty-iconPistons">🏀</div>
          <div className="empty-titlePistons">No Players in Roster</div>
          <div className="empty-subtitlePistons">
            Currently no players are assigned to the Pistons team
          </div>
        </div>
      )}
    </div>
