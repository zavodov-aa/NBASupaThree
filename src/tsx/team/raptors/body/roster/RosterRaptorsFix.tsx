...
const RosterRaptors: React.FC = () => {
    ...
    const fetchPlayers = async () => {
      try {
        ...
        const { data: mainData, error: mainError } = await supabase
          .from("Players")
          .select("*")
          .eq("team", "Toronto Raptors"); // Изменено на Toronto Raptors
        ...
        const { data: gLeagueData, error: gLeagueError } = await supabase
          .from("Players")
          .select("*")
          .eq("g_league", "Toronto Raptors"); // Изменено на Toronto Raptors
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr
        key={player.id}
        className={isFranchisePlayer ? "franchiseRaptors" : ""}
      >
        <td>
          <div className="number-cellRaptors">
            {index + 1}
            {isFranchisePlayer && <span className="starRaptors">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagRaptors">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightRaptors">
  ...
  if (loading) return <div className="loadingRaptors">Loading Roster...</div>;

  const hasPlayers =
    franchisePlayer || mainRoster.length > 0 || gLeagueRoster.length > 0;

  return (
    <div className="rosterRaptors">
      <div className="roster-headerRaptors">
        <h1>RAPTORS ROSTER</h1> {/* Изменено на RAPTORS */}
        <div className="roster-subtitleRaptors">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerRaptors">
          <table className="roster-tableRaptors">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerRaptors">
                  <td colSpan={14}>
                    <div className="g-league-headerRaptors">
                      G-LEAGUE ROSTER
                    </div>
                  </td>
                </tr>
        ...
        <div className="empty-stateRaptors">
          <div className="empty-iconRaptors">🏀</div>
          <div className="empty-titleRaptors">No Players in Roster</div>
          <div className="empty-subtitleRaptors">
            Currently no players are assigned to the Raptors team{" "}
            {/* Изменено на Raptors */}
          </div>
        </div>
      )}
