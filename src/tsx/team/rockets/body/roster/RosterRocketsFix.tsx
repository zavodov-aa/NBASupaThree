...
const RosterRockets: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr
        key={player.id}
        className={isFranchisePlayer ? "franchiseRockets" : ""}
      >
        <td>
          <div className="number-cellRockets">
            {index + 1}
            {isFranchisePlayer && <span className="starRockets">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagRockets">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightRockets">
  ...
  return (
    <div className="rosterRockets">
      <div className="roster-headerRockets">
        <h1>ROCKETS ROSTER</h1>
        <div className="roster-subtitleRockets">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerRockets">
          <table className="roster-tableRockets">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerRockets">
                  <td colSpan={14}>
                    <div className="g-league-headerRockets">
                      G-LEAGUE ROSTER
                    </div>
                  </td>
                </tr>
        ...
        <div className="empty-stateRockets">
          <div className="empty-iconRockets">🏀</div>
          <div className="empty-titleRockets">No Players in Roster</div>
          <div className="empty-subtitleRockets">
            Currently no players are assigned to the Rockets team
          </div>
        </div>
      )}
    </div>
