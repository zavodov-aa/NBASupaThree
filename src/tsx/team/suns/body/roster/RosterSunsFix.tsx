...
const RosterSuns: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr key={player.id} className={isFranchisePlayer ? "franchiseSuns" : ""}>
        <td>
          <div className="number-cellSuns">
            {index + 1}
            {isFranchisePlayer && <span className="starSuns">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagSuns">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightSuns">
  ...
  return (
    <div className="rosterSuns">
      <div className="roster-headerSuns">
        <h1>SUNS ROSTER</h1>
        <div className="roster-subtitleSuns">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerSuns">
          <table className="roster-tableSuns">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerSuns">
                  <td colSpan={14}>
                    <div className="g-league-headerSuns">G-LEAGUE ROSTER</div>
                  </td>
                </tr>
              )}
        ...
        <div className="empty-stateSuns">
          <div className="empty-iconSuns">🏀</div>
          <div className="empty-titleSuns">No Players in Roster</div>
          <div className="empty-subtitleSuns">
            Currently no players are assigned to the Suns team
          </div>
        </div>
      )}
    </div>
