...
const RosterThunder: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr
        key={player.id}
        className={isFranchisePlayer ? "franchiseThunder" : ""}
      >
        <td>
          <div className="number-cellThunder">
            {index + 1}
            {isFranchisePlayer && <span className="starThunder">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagThunder">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightThunder">
  ...
  return (
    <div className="rosterThunder">
      <div className="roster-headerThunder">
        <h1>THUNDER ROSTER</h1>
        <div className="roster-subtitleThunder">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerThunder">
          <table className="roster-tableThunder">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerThunder">
                  <td colSpan={14}>
                    <div className="g-league-headerThunder">
                      G-LEAGUE ROSTER
                    </div>
                  </td>
                </tr>
        ...
        <div className="empty-stateThunder">
          <div className="empty-iconThunder">🏀</div>
          <div className="empty-titleThunder">No Players in Roster</div>
          <div className="empty-subtitleThunder">
            Currently no players are assigned to the Thunder team
          </div>
        </div>
      )}
    </div>
