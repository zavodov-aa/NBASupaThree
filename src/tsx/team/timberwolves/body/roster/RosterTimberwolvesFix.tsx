...
import "./rosterTimberwolvesFix.css";
...
const RosterTimberwolves: React.FC = () => {
    ...
    const fetchPlayers = async () => {
      try {
        ...
        const { data: mainData, error: mainError } = await supabase
          .from("Players")
          .select("*")
          .eq("team", "Minnesota Timberwolves");
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr
        key={player.id}
        className={isFranchisePlayer ? "franchiseTimberwolves" : ""}
      >
        <td>
          <div className="number-cellTimberwolves">
            {index + 1}
            {isFranchisePlayer && <span className="starTimberwolves">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagTimberwolves">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightTimberwolves">
  ...
  if (loading)
    return <div className="loadingTimberwolves">Loading Roster...</div>;

  const hasPlayers =
    franchisePlayer || mainRoster.length > 0 || gLeagueRoster.length > 0;

  return (
    <div className="rosterTimberwolves">
      <div className="roster-headerTimberwolves">
        <h1>TIMBERWOLVES ROSTER</h1>
        <div className="roster-subtitleTimberwolves">
          ACTIVE PLAYERS & CONTRACTS
        </div>
      </div>

      {hasPlayers ? (
        <div className="table-containerTimberwolves">
          <table className="roster-tableTimberwolves">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerTimberwolves">
                  <td colSpan={14}>
                    <div className="g-league-headerTimberwolves">
                      G-LEAGUE ROSTER
                    </div>
                  </td>
                </tr>
        ...
        <div className="empty-stateTimberwolves">
          <div className="empty-iconTimberwolves">🏀</div>
          <div className="empty-titleTimberwolves">No Players in Roster</div>
          <div className="empty-subtitleTimberwolves">
            Currently no players are assigned to the Timberwolves team
          </div>
        </div>
      )}
    </div>
...
export default RosterTimberwolves;
