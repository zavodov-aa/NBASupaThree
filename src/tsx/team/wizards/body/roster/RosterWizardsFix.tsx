...
const RosterWizards: React.FC = () => {
  ...
  const renderPlayerRow = (player: Player, index: number) => {
    ...
    return (
      <tr
        key={player.id}
        className={isFranchisePlayer ? "franchiseWizards" : ""}
      >
        <td>
          <div className="number-cellWizards">
            {index + 1}
            {isFranchisePlayer && <span className="starWizards">★</span>}
          </div>
        </td>
        <td>
          <span className="player-tagWizards">{player.active_roster}</span>
        </td>
        <td>{player.position}</td>
        <td>{player.pos_elig}</td>
        <td>{player.age || "—"}</td>
        <td className="highlightWizards">
  ...
  return (
    <div className="rosterWizards">
      <div className="roster-headerWizards">
        <h1>WIZARDS ROSTER</h1>
        <div className="roster-subtitleWizards">ACTIVE PLAYERS & CONTRACTS</div>
      </div>

      {hasPlayers ? (
        <div className="table-containerWizards">
          <table className="roster-tableWizards">
            <tbody>
              ...
              {gLeagueRoster.length > 0 && (
                <tr className="g-league-dividerWizards">
                  <td colSpan={14}>
                    <div className="g-league-headerWizards">
                      G-LEAGUE ROSTER
                    </div>
                  </td>
                </tr>
        ...
        <div className="empty-stateWizards">
          <div className="empty-iconWizards">🏀</div>
          <div className="empty-titleWizards">No Players in Roster</div>
          <div className="empty-subtitleWizards">
            Currently no players are assigned to the Wizards team
          </div>
        </div>
      )}
    </div>
